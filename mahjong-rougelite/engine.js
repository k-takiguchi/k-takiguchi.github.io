...
(function (root, factory) {
  ...
  // ★D78: コンテンツテーブル(妖怪/道具/メタ進行/ラウンド定義等)は data.js(MJ_DATA)に分離。
  const MJ_DATA = (typeof module !== "undefined" && module.exports) ? require("./data.js") : root.MJ_DATA;
  const api = factory(MJHand, MJ_DATA);
  if (typeof module !== "undefined" && module.exports) module.exports = api;
  root.MJ = api;
})(typeof self !== "undefined" ? self : this, function (MJHand, MJ_DATA) {
  "use strict";
  ...
  // ★D78: 実データは data.js(MJ_DATA.yokai/yokaiIds)へ分離。ここでは定数名に束縛するのみ（参照コードは不変）。
  const YOKAI = MJ_DATA.yokai;
  const YOKAI_IDS = MJ_DATA.yokaiIds;
  // ---- ボス妖怪ギミック（A1: ボスブラインド相当） ---------------------------
  // 参照: ../design/boss-gimmicks-a1.md
  // ★D78: 実データは data.js(MJ_DATA.bossGimmicks)へ分離。ここでは定数名に束縛するのみ（参照コードは不変）。
  const BOSS_GIMMICKS = MJ_DATA.bossGimmicks;
  ...
  // ★D78: 実データは data.js(MJ_DATA.endlessGimmickPool)へ分離。
  const ENDLESS_POOL = MJ_DATA.endlessGimmickPool;
  ...
  // ★D78: 実データは data.js(MJ_DATA.items)へ分離。ここでは定数名に束縛するのみ（参照コードは不変）。
  const ITEMS = MJ_DATA.items;
  const ITEM_IDS = Object.keys(ITEMS);

  // ---- メタ進行（恒久強化） --------------------------------------------------
  // ★D50 価格改定: 1Lv=5〜10プレイ相当（1周クリア≒19メダル基準）。
  // late:true の枠開放系は「半分エンドコンテンツ」としてガッツリ高額（15〜25プレイ相当）。
  // ★D78: 実データは data.js(MJ_DATA.metaUpgrades/metaIds)へ分離。ここでは定数名に束縛するのみ（参照コードは不変）。
  const META_UPGRADES = MJ_DATA.metaUpgrades;
  const META_IDS = MJ_DATA.metaIds;
  ...
  // ★D78: 実データは data.js(MJ_DATA.campaignRounds)へ分離。呼び出しごとに独立した配列/オブジェクトを返す
  // （旧実装は関数内リテラルで毎回新規生成していたため、その挙動を保つ＝呼び出し元でのミューテーションに対して独立）。
  function makeRounds() {
    return MJ_DATA.campaignRounds.map((r) => Object.assign({}, r));
  }
  const CAMPAIGN_ROUNDS = makeRounds();
  // ★D78: 実データは data.js(MJ_DATA.endlessEnemyNames)へ分離。
  const ENDLESS_NAMES = MJ_DATA.endlessEnemyNames;
